...
function calculateStreak(habit: Habit, today: Date): number {
  ...
  // Create a set of completed dates stringified for fast lookup (YYYY-MM-DD)
  ...
  // If today is completed, start streak from today, otherwise start from yesterday
  if (completedDates.has(todayStr)) {
    streak++
  } else {
    // It's fine if today isn't done yet, check yesterday
  }
...
export function HabitsPage() {
  ...
  const handleToggle = (habitId: string, date: Date) => {
    // Only toggle up to today, not future
    const today = new Date()
    today.setHours(23, 59, 59, 999)
    if (date > today) return

    // Convert date to YYYY-MM-DD local time string for the API
    // Need to avoid timezone shifts. 
